...
def on_mqtt_message(
    ...
    try:
        ...
        if msg == "online" and userdata:
            gracetime = CONFIG.get(CFG_HASS_BIRTH_GRACETIME, 15)
            _LOGGER.info(
...
def mqtt_start(hass: hass_int.HassIntegration | None = None) -> mqtt_client.Client | None:
    ...
    try:
        ...
        client.username_pw_set(CONFIG[CFG_MQTT_LOGIN], CONFIG[CFG_MQTT_PASSWORD])
        client.connect(CONFIG[CFG_MQTT_SERVER], CONFIG[CFG_MQTT_PORT], keepalive=60)

        if hass:
            client.subscribe(topic=CONFIG[CFG_HASS_BASE_TOPIC] + "/status")
...
def mqtt_publish(topic: str, payload: str) -> None:
    ...
    if CONFIG[CFG_MQTT_DISABLE]:  # Don't do anything - just logg
        _LOGGER.info("- %s: %s", topic, str(payload))
    else:
        auth = {"username": CONFIG[CFG_MQTT_LOGIN], "password": CONFIG[CFG_MQTT_PASSWORD]}
        _LOGGER.debug("- %s: %s", topic, str(payload))
        try:
            publish.single(
                topic=topic,
                payload=payload,
                hostname=CONFIG[CFG_MQTT_SERVER],
                port=CONFIG[CFG_MQTT_PORT],
